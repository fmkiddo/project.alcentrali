...
class ClientRequest extends BaseRESTController {
	...
	private function decodeAuth () {
		$authHeader	= $this->request->getHeader ('Authorization')->getValue ();
		$authString	= str_replace('Basic ', '', $authHeader);
		$authData	= explode('#', json_decode(base64_decode(base64_decode($authString))));
		return $authData;
	}
	...
	private function authorizationProcess () {
		...
		if ($this->request->getMethod(TRUE) !== 'PUT') 
			...
			$clientAuth = $this->decodeAuth();
			$model		= new \App\Models\ClientModel ();
			$client		= $model->find ($clientAuth[0]);
			$verified	= TRUE;
			$clientdata	= '';
